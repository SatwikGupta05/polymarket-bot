...
async def analyze_ai_costs(db_manager: DatabaseManager) -> Dict:
    ...
    if today_cost > settings.trading.daily_ai_budget * 0.8:
        recommendations.append(f"[WARN]  Near daily budget limit: ${today_cost:.3f} / ${settings.trading.daily_ai_budget}")
    ...
    if weekly_analyses > weekly_decisions * 3:  # Too many analyses relative to decisions
        recommendations.append("[REFRESH] High analysis-to-decision ratio - improve filtering")
    ...
    async with aiosqlite.connect(db_manager.db_path) as db:
        ...
        cursor = await db.execute("""
            SELECT COUNT(*) as open_positions,
                   AVG((julianday('now', 'localtime') - julianday(timestamp)) * 24) as avg_hours_held
