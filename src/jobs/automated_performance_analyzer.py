...
class AutomatedPerformanceAnalyzer:
    ...
    async def _gather_portfolio_data(self) -> Dict[str, Any]:
        """Gather current portfolio state from the Polymarket API."""
    ...
    async def _generate_grok_analysis(self, portfolio_data: Dict[str, Any], metrics: PerformanceMetrics, risk_checks: List[RiskCheck]) -> Dict[str, Any]:
        ...
        analysis_prompt = f"""
You are an expert quantitative trading analyst. Analyze this Polymarket prediction market trading system:
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        if args.save_report:
            ...
            with open(filename, 'w') as f:
                json.dump(report, f, indent=2, default=str)
            print(f"\n[SAVE] Detailed report saved: {filename}")
