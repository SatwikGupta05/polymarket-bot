...
async def make_decision_for_market(
    ...
    try:
            ...
            if market.yes_price >= settings.trading.high_confidence_market_odds:
                ...
                if decision.side == "YES" and decision.confidence >= settings.trading.high_confidence_threshold:
                    ...
                    if quantity > 0:
                        ...
                        position = Position(
                            market_id=market.market_id.strip().lower(),
                            side=decision.side,
                            entry_price=market.yes_price,
                            quantity=quantity,
        ...
        _has_paid = bool(settings.api.xai_api_key or settings.api.openrouter_api_key)
        sentiment_analysis = _has_paid and (
            getattr(settings, 'sentiment_analysis', False) or (
        ...
        if market.yes_price is None or market.no_price is None:
            logger.warning(f"Missing price data for {market.market_id}. Skipping decision.")
            await db_manager.record_market_analysis(
                market.market_id, "SKIP", 0.0, total_analysis_cost, "missing_price_data"
            )
            return None

        skip_news_for_low_volume = getattr(settings.trading, "skip_news_for_low_volume", False)
        news_search_volume_threshold = getattr(
            settings.trading,
        ...
        if skip_news_for_low_volume and market.volume < news_search_volume_threshold:
            ...
            if not sentiment_analysis or news_summary is None:
                # Free-tier: use basic keyword context (no API cost)
                news_summary = (
                    f"Market: {market.title}. "
                    f"YES price: {market.yes_price:.2f}. NO price: {market.no_price:.2f}. "
                    f"Volume: {market.volume}. Analyze based on this data."
                )
                estimated_search_cost = 0.0
        ...
        if decision.action == "BUY" and decision.confidence >= settings.trading.min_confidence_to_trade:
            ...
            if not can_add_position:
                ...
                logger.info(f"[WARN] Position size ${initial_position_value:.2f} exceeds limits, attempting to reduce...")
