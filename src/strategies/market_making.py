...
class AdvancedMarketMaker:
    ...
    def __init__(
        ...
        self.min_spread = getattr(settings.trading, 'min_spread_for_making', 0.01)  # $0.03 minimum
    ...
    async def analyze_market_making_opportunities(
        ...
        for market in markets:
            try:
                ...
                # Prices are already normalized to 0.0-1.0 by PolymarketClient.
                current_yes_price = float(
                    market_info.get('yes_bid_dollars', 0) or market_info.get('yes_price', 0) or 0
                )
                current_no_price = float(
                    market_info.get('no_bid_dollars', 0) or market_info.get('no_price', 0) or 0
                )
                ...
                # When AI is unavailable, downgrade to spread-only market making instead
                # of forcing a neutral 0.5/low-confidence analysis through edge filters.
                if fallback_used or yes_edge_result.passes_filter or no_edge_result.passes_filter or (
                    yes_edge_result.edge_percentage > 0.02 or no_edge_result.edge_percentage > 0.02
                ):
                    ...
                    if opportunity and opportunity.total_expected_profit >= 0:
                        ...
                        if fallback_used:
                            self.logger.info(
                                f"MARKET MAKING APPROVED (fallback): {market.market_id} - "
                                f"spread-only mode, confidence: {ai_confidence:.1%}"
                            )
                        else:
                            self.logger.info(
    ...
    async def _place_market_making_orders(self, opportunity: MarketMakingOpportunity):
        ...
        # Place orders through the compatibility client (simulated for now)
        for order in orders:
    ...
    async def _place_limit_order(self, order: LimitOrder):
        ...
        try:
            ...
            if live_mode:
                ...
                # Convert side to the compatibility client's expected format
                ...
                paper_position = Position(
                    ...
                    side=order.side,
                    entry_price=order.price / 100,
                    quantity=order.quantity,
                    timestamp=datetime.now(timezone.utc),
                    rationale=(
                        f"Paper market making order placed at {order.price / 100:.3f} "
                        f"with expected profit ${order.expected_profit:.2f}"
                    ),
                    confidence=0.40,
                    live=False,
                    status="open",
                ...
                await self.db_manager.add_position(paper_position)

                self.logger.info(
                    f"[LOG] SIMULATED limit order placed: {order.side} {order.quantity} at {order.price:.1f}¢ "
    ...
    async def _get_ai_analysis(self, market: Market) -> Optional[Dict]:
        ...
        try:
            ...
            if response is None:
                self.logger.info(
                    f"AI analysis unavailable for {market.market_id} due to API limits, "
                    f"downgrading to spread-only fallback"
                )
                return self._get_market_making_fallback(market, "API unavailable")
            ...
            self.logger.warning(
                f"AI analysis failed for {market.market_id}, downgrading to spread-only fallback"
            )
            return self._get_market_making_fallback(market, "AI analysis failed")
                 
        except Exception as e:
            self.logger.error(f"Error getting AI analysis: {e}")
            return self._get_market_making_fallback(market, "Error in analysis")

    def _get_market_making_fallback(self, market: Market, reason: str) -> Dict:
        """Fallback used when AI is unavailable: keep spread capture alive with modest confidence."""
        implied_prob = getattr(market, "yes_price", 0.5) or 0.5
        implied_prob = max(0.05, min(0.95, float(implied_prob)))

        return {
            'probability': implied_prob,
            'confidence': 0.40,
            'volatility_factors': reason,
            'stability': 0.6,
            'fallback_used': True,
        }
    ...
    async def _should_update_order(self, order: LimitOrder) -> bool:
        ...
        try:
            ...
            market_info = market_data.get('market', market_data)
            current_yes_price = market_info.get('yes_price')
            if current_yes_price is None:
                self.logger.warning(f"Missing price data for {order.market_id}, skipping order update check.")
                return False
