...
Integrated with the repo's Polymarket client compatibility layer and DatabaseManager.
...
class SafeCompounder:
    """
    Edge-based NO-side strategy integrated with the repo's Polymarket client.
    ...
    async def run(self, dry_run: Optional[bool] = None) -> Dict:
        ...
        print("\n[SIGNAL] Step 1: Fetching all active markets...", flush=True)
        ...
        print(f"\n[STATS] Step 3: Checking orderbooks for edge ≥ ${self.min_edge:.2f}...", flush=True)
        ...
        print(f"\n[LIST] Top Opportunities:", flush=True)
        ...
        print(f"\n{'='*70}", flush=True)
        print(f"[STATS] SAFE COMPOUNDER REPORT", flush=True)
    ...
    async def _place_resting_orders(
        ...
        for opp in opportunities:
            ...
            if self.dry_run:
                ...
                print(
                    f"  [LABEL] [DRY] NO x{contracts} @ ${price:.2f} | "
            ...
            for o in yes_orders:
                try:
                    ...
                    if isinstance(yes_price, (int, float)) and yes_price > 0:
                        ...
                        price_display = "?"
                    print(
                        f"  [DELETE] Cancelled YES: {o['ticker']} @ {price_display}",
                        flush=True,
                    )
                    cancelled += 1
    ...
    async def check_fills(self) -> None:
        ...
        try:
            ...
            print(
                f"[LIST] Resting: {len(no_resting)} NO, {len(yes_resting)} YES",
                flush=True,
            )
        except Exception:
            pass
        ...
        try:
            ...
            print(f"\n[STATS] Last 20 fills:", flush=True)
            for f in fill_list:
                ticker = f.get("ticker", "")
                side = f.get("side", "")
                count = f.get("count", 0)
