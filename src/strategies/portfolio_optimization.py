...
class MarketOpportunity:
    ...
    @property
    def ticker(self) -> str:
        """Alias for market_id to support legacy callers."""
    ...
    def _multi_objective_optimization(
        ...
        try:
            ...
            if not SCIPY_AVAILABLE or minimize is None:
                if not hasattr(self.__class__, '_scipy_warned'):
                    self.logger.warning(
                        "scipy not installed - using simple allocation. "
                        "pip install scipy for better risk-adjusted returns."
                    )
                    self.__class__._scipy_warned = True
...
async def create_market_opportunities_from_markets(
    ...
    max_markets_to_analyze = 5  # Reduced free-tier AI fan-out to avoid provider rate limits
    ...
    for market in markets:
        try:
            ...
            # Skip markets with missing or clearly invalid data before deeper processing.
            if not market_id:
                continue
            ...
            market_prob = market_info.get('yes_bid_dollars')
            if market_prob is None and market_info.get('yes_bid') is not None:
                market_prob = market_info.get('yes_bid') / 100
            ...
            # If AI analysis failed, use a lightweight fallback instead of dropping the market.
            if predicted_prob is None or confidence is None:
                fallback_action = fallback_decision(market)
                if fallback_action != "BUY":
                    logger.warning(f"AI analysis failed for {market.market_id}, skipping")
                    continue
                predicted_prob = min(0.95, market_prob + 0.12)
                confidence = 0.55
                logger.info(f"AI analysis failed for {market.market_id}, using volume-based fallback decision")
            ...
            logger.warning(f"Skipping market {getattr(market, 'market_id', '?')}: {e}")
            continue
...
async def _evaluate_immediate_trade(
    ...
    try:
        ...
        try:
            ...
            if active_positions:
                logger.info(f"[STATS] Active positions: {len(active_positions)}")
        ...
        if not can_add_position:
            ...
            logger.info(f"[WARN] Position size ${initial_position_size:.2f} exceeds limits, attempting to reduce...")
        ...
        if position_count > 0:
            logger.info(f"[SKIP] Skipping immediate trade for {opportunity.market_id} - position already exists")
            return
        ...
        if min_cost > available_cash:
            logger.info(f"[SKIP] Cannot afford minimum 1 contract: ${min_cost:.2f} > ${available_cash:.2f}")
            return
            
        logger.info(f"[STATS] Trade details: {shares} {side} shares @ ${entry_price:.2f} = ${min_cost:.2f}")
        ...
        try:
            ...
            # Tradeable markets should report active/open status through the compatibility layer
            if market_status not in ['active', 'open']:
                logger.warning(f"[SKIP] Skipping {opportunity.market_id} - Market status: {market_status} (not active/open)")
                return
            
            if not (yes_ask and no_ask and yes_ask > 0 and no_ask > 0):
                logger.warning(f"[SKIP] Skipping {opportunity.market_id} - No valid prices (YES={yes_ask}¢, NO={no_ask}¢)")
                return
                
            logger.info(f"[OK] Market validation passed for {opportunity.market_id} - Status: {market_status}, proceeding with trade!")
            
        except Exception as e:
            logger.error(f"[SKIP] Skipping {opportunity.market_id} - Market validation failed: {e}")
            import traceback
