...
def _clean_env_value(name: str, default: str = "") -> str:
    """Treat placeholder secrets as unset so provider routing can fall back cleanly."""
...
class APIConfig:
    ...
    # Legacy compatibility aliases retained for older helper paths.
    # Validation no longer depends on these values.
    kalshi_api_key: str = field(default_factory=lambda: _clean_env_value("KALSHI_API_KEY", ""))
    kalshi_base_url: str = field(default_factory=lambda: os.getenv("KALSHI_BASE_URL", "https://api.elections.kalshi.com/trade-api/v2").strip())

    # ── Polymarket primary configuration ────────────────────────────────
    polygon_private_key: str = field(default_factory=lambda: _clean_env_value("POLYGON_PRIVATE_KEY", ""))
    polygon_public_key:  str = field(default_factory=lambda: _clean_env_value("POLYGON_PUBLIC_KEY",  ""))
    polymarket_clob_url: str = "https://clob.polymarket.com"
    polymarket_gamma_url: str = "https://gamma-api.polymarket.com"

    # ── AI providers ─────────────────────────────────────────────────────
    # Free tier (demo-ready — no credit card needed)
    groq_api_key:    str = field(default_factory=lambda: _clean_env_value("GROQ_API_KEY", ""))
    gemini_api_key:  str = field(default_factory=lambda: _clean_env_value("GEMINI_API_KEY", ""))

    # Paid tier (optional — higher accuracy)
    xai_api_key:          str = field(default_factory=lambda: _clean_env_value("XAI_API_KEY", ""))
    openrouter_api_key:   str = field(default_factory=lambda: _clean_env_value("OPENROUTER_API_KEY", ""))
    openai_api_key:       str = field(default_factory=lambda: _clean_env_value("OPENAI_API_KEY", ""))
...
class SentimentConfig:
    ...
    rss_feeds: List[str] = field(default_factory=lambda: [
        # Reuters removed (DNS fails in many environments)
        "https://feeds.bbci.co.uk/news/world/rss.xml",
        "https://feeds.bbci.co.uk/news/business/rss.xml",
        "https://rss.nytimes.com/services/xml/rss/nyt/Business.xml",
        "https://rss.nytimes.com/services/xml/rss/nyt/Technology.xml",
        "https://hnrss.org/frontpage",   # Hacker News - always up
    ])
...
class TradingConfig:
    ...
    max_position_size_pct: float = 15.0  # Demo mode: allow larger sizing
    max_daily_loss_pct: float = 10.0    # SANE: 10% daily loss limit (was 15%)
    max_positions: int = 25              # Demo mode: allow more concurrent positions
    ...
    max_time_to_expiry_days: int = 365   # Demo mode: allow long-dated markets
    
    # AI decision making — DATA-DRIVEN THRESHOLDS  
    min_confidence_to_trade: float = 0.40   # Demo mode: more permissive trading threshold
    ...
    primary_model: str = "groq"  # Free-tier default provider for forecasting
    fallback_model: str = "gemini"  # Free-tier fallback provider
    ...
    analysis_cooldown_hours: int = 0  # PAPER MODE: 0 = never skip due to cooldown
    max_analyses_per_market_per_day: int = 99  # PAPER MODE: no daily limit per market
    ...
    min_volume_for_ai_analysis: float = 0.0  # Demo mode: analyze all market volumes
...
# total_capital: DYNAMICALLY FETCHED from exchange balance - never hardcoded!
...
min_volume_for_analysis: float = 150.0  # DECREASED: Much lower minimum volume (was 1000, now 200)
...
class Settings:
    ...
    def validate(self) -> bool:
        ...
        # Polymarket: wallet keys are only required for live trading
        # Paper trading works without them
        has_ai = (
            self.api.groq_api_key or
            self.api.gemini_api_key or
            self.api.xai_api_key or
            self.api.openrouter_api_key
        )
        if not has_ai:
            print("[WARN]  WARNING: No AI API keys found. Set GROQ_API_KEY or GEMINI_API_KEY for free tier.")
