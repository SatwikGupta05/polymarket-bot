...
class CashReservesManager:
    ...
    def _get_cash_recommendations(self, reserve_pct: float) -> List[str]:
        ...
        if reserve_pct < self.critical_threshold_pct:
            ...
            recommendations.append("[WARN] EMERGENCY: Close 2-3 positions immediately")
            recommendations.append("[WARN] Suspend new trading until above 15%")
        elif reserve_pct < self.minimum_reserve_pct:
            recommendations.append("[WARN] Close some positions to build reserves")
            recommendations.append("[WARN] Avoid new trades until above 15%")
