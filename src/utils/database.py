...
class DatabaseManager(TradingLoggerMixin):
    ...
    def __init__(self, db_path: str = "trading_system.db"):
        ...
        self._sqlite_timeout_seconds = 30
        self._sqlite_busy_timeout_ms = 30000
    ...
    async def _configure_connection(self, db: aiosqlite.Connection) -> None:
        """Apply connection-level SQLite settings to reduce lock contention."""
    ...
    async def initialize(self) -> None:
        ...
        async with aiosqlite.connect(self.db_path, timeout=self._sqlite_timeout_seconds) as db:
    ...
    async def get_market(self, market_id: str):
        """
        Try multiple identifier formats to find the market.
        Handles condition_id, slug, and hex-format mismatches.
        """
        if not market_id:
            return None
        ...
        async with aiosqlite.connect(self.db_path) as db:
            ...
            # Try 2: strip 0x prefix and try both stripped and prefixed forms.
            if market_id.startswith("0x"):
                stripped = market_id[2:]
                cursor = await db.execute(
                    "SELECT * FROM markets WHERE market_id = ? OR market_id = ?",
            ...
            # Try 3: partial suffix match for legacy condition-id mismatches.
            try:
                cursor = await db.execute(
                    "SELECT * FROM markets WHERE market_id LIKE ?",
                    (f"%{market_id[-20:]}%",)
                )
                row = await cursor.fetchone()
                if row:
    ...
    async def record_market_analysis(
        ...
        for attempt in range(max_retries):
            try:
                ...
                self.logger.warning(
                    "Database locked while recording market analysis; retrying",
                    market_id=market_id,
                    attempt=attempt + 1,
