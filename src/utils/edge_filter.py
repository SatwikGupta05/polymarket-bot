...
class EdgeFilter:
    ...
    MIN_EDGE_REQUIREMENT = 0.02        # Demo mode: lower edge floor
    HIGH_CONFIDENCE_EDGE = 0.02        # 2% edge for high confidence
    MEDIUM_CONFIDENCE_EDGE = 0.03      # 3% edge for medium confidence
    LOW_CONFIDENCE_EDGE = 0.05         # 5% edge for low confidence
    
    # DECREASED: More permissive filters for more opportunities
    MIN_CONFIDENCE_FOR_TRADE = 0.40    # Demo mode: lower confidence floor
    ...
    def calculate_edge(
        ...
        if confidence >= 0.8:
            required_edge = cls.HIGH_CONFIDENCE_EDGE     # 2% for high confidence
        elif confidence >= 0.6:
            required_edge = cls.MEDIUM_CONFIDENCE_EDGE   # 3% for medium confidence
        else:
            required_edge = cls.LOW_CONFIDENCE_EDGE      # 5% for low confidence
