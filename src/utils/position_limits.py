...
class PositionLimitsManager:
    ...
    def __init__(self, db_manager: DatabaseManager, kalshi_client: KalshiClient):
        ...
        self.max_positions = 60  # Demo mode: allow many more concurrent positions
        self.max_position_size_pct = 7.0  # INCREASED: 5% max per trade (was 3%)
    ...
    async def _get_positions_for_closure(self, count: int) -> List[PositionToClose]:
        ...
        try:
            ...
            for position in positions:
                ...
                age_hours = self._hours_since(position.timestamp)
