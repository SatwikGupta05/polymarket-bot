...
ALLOWED_MODELS = set(MODEL_PRICING.keys())
...
class OpenRouterClient(TradingLoggerMixin):
    ...
    def _load_api_keys(self, explicit_api_key: Optional[str]) -> List[str]:
        ...
        if explicit_api_key:
            cleaned = self._clean_key(explicit_api_key)
            if cleaned:
                keys.append(cleaned)

        primary = self._clean_key(settings.api.openrouter_api_key)
        if primary:
            keys.append(primary)
    ...
    def _rotate_api_key(self) -> bool:
        """Switch to the next OpenRouter key if one exists."""
    ...
    def _validate_model(model: str) -> str:
        assert model in ALLOWED_MODELS, f"Unsupported OpenRouter model: {model}"
        return model
    ...
    def _build_fallback_chain(self, requested_model: Optional[str] = None) -> List[str]:
        ...
        """
        first = self._validate_model(requested_model or self.default_model)
        chain = [first]
    ...
    async def _request_single_model(
        ...
        for attempt in range(self.MAX_RETRIES_PER_MODEL):
            try:
                ...
                if self._should_rotate_key(exc) and self._rotate_api_key():
                    continue
