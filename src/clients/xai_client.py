...
class XAIClient(TradingLoggerMixin):
    ...
    def __init__(self, api_key: Optional[str] = None, db_manager=None):
        ...
        self.logger.info(
            ...
            mode="xai" if self.api_key else "free_tier",
            primary_model=self.primary_model,
            effective_primary_model=self.primary_model if self.api_key else "groq->gemini",
    ...
    def _is_configuration_error(error: Exception) -> bool:
        """Return True for non-retryable config/model selection errors."""
        error_str = str(error).lower()
        return any(indicator in error_str for indicator in [
            "model not found",
            "unsupported xai model",
            "unsupported model",
            "invalid model",
        ])
    ...
    def _validate_model(model: str) -> str:
        assert model in ALLOWED_MODELS, f"Unsupported xAI model: {model}"
        return model
...
Provide a brief, factual summary under {max_length//2} words. If no current information is available, clearly state that."""
    ...
    @staticmethod
    def _default_hold_decision(reasoning: str = "Default hold decision due to parsing or model failure.") -> TradingDecision:
        """Return a safe default decision when model output cannot be used."""
    ...
    @staticmethod
    def _shutdown_cancellation_requested() -> bool:
        """Return True when the current task is being actively cancelled by the event loop."""
        ...
        try:
            for attempt in range(2):
                try:
                    ...
                    if not decision:
                        raise RuntimeError("Critical: No trading decisions")

                    return decision
                except asyncio.CancelledError:
                    if self._shutdown_cancellation_requested() or attempt == 1:
                        self.logger.warning("Trading decision task cancelled; propagating cancellation")
                        raise
                    self.logger.warning("Task cancelled during trading decision, retrying once...")

            # FIX 3: fallback decision when AI is unavailable
            # Instead of skipping entirely, make a conservative directional bet
            # based purely on market price (contrarian - buy cheap NO on overpriced markets)
            self.logger.warning("AI unavailable - using price-based fallback decision")

            yes_price = float(market_data.get("yes_bid_dollars", 0.5) or 0.5)
            ...
            if yes_price > 0.80:
                return TradingDecision(
                    ...
                    side="NO",
                    confidence=0.52,
                    limit_price=int((1 - yes_price) * 100),
                )
            elif yes_price < 0.20:
                return TradingDecision(
                    action="BUY",
    ...
    async def _get_trading_decision_with_prompt(
        ...
        try:
            ...
            if not self.api_key:
                from src.clients.free_model_client import get_free_completion

                response_text = await get_free_completion(prompt)
...
{{"action": "BUY|SKIP", "side": "YES|NO", "limit_price": 1-99, "confidence": 0.0-1.0, "reasoning": "brief explanation"}}"""
    ...
    async def _make_completion_request(
        ...
        model_to_use = self._validate_model(model_to_use)
        ...
        for attempt in range(max_retries):
            try:
                ...
                chat = self.client.chat.create(
                    model=self._validate_model(model_to_use),
                ...
                if self._is_configuration_error(e):
                    self.logger.warning(
                        "Non-retryable configuration error; stopping retries",
                        model=model_to_use,
                        error=str(e),
                    )
                    break
        ...
        self.logger.error(
            "xAI completion aborted before retries completed",
            model=model_to_use,
            reason="configuration_error_or_unrecoverable_failure",
        )
        return None, 0.0
    ...
    async def _try_fallback_model(
        ...
        try:
            ...
            fallback_model = self._validate_model(fallback_model)
    ...
    async def get_completion(
        ...
        Falls back to free tier (Groq / Gemini) when XAI_API_KEY is absent.
        ...
        if not self.api_key:
            try:
                from src.clients.free_model_client import get_free_completion
                return await get_free_completion(prompt)
            except Exception as e:
                self.logger.warning(f"Free-tier completion failed: {e}")
                return None

        # ── Paid path (original logic) ────────────────────────────────────
        try:
            ...
            # Last-resort free-tier fallback
            try:
                from src.clients.free_model_client import get_free_completion
                return await get_free_completion(prompt)
            except Exception:
