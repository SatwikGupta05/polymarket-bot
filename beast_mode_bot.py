...
class BeastModeBot:
    """
    Beast Mode Trading Bot - Advanced Multi-Strategy Trading System [START]
    ...
    def __init__(self, live_mode: bool = False, dashboard_mode: bool = False):
        ...
        self.logger.info(
            f"[START] Beast Mode Bot initialized - "
            f"Mode: {'LIVE TRADING' if live_mode else 'PAPER TRADING'}"
        )
        self.logger.info(f"[STATS] CLI live_mode parameter: {live_mode}")
        self.logger.info(f"[SETTINGS] settings.trading.live_trading_enabled: {settings.trading.live_trading_enabled}")
        self.logger.info(f"[SETTINGS] settings.trading.paper_trading_mode: {settings.trading.paper_trading_mode}")
        
        if live_mode:
            self.logger.warning("[WARN] LIVE TRADING MODE ENABLED - REAL MONEY WILL BE USED")
            self.logger.warning("[WARN] All orders will be placed on Polymarket")
        else:
            self.logger.info("[LOG] Paper trading mode - orders will be simulated")

    async def run_dashboard_mode(self):
        """Run in live dashboard mode with real-time updates."""
        try:
            self.logger.info("[START] Starting Beast Mode Dashboard Mode")
            dashboard = BeastModeDashboard()
            await dashboard.show_live_dashboard()
        except KeyboardInterrupt:
            self.logger.info("[WAVE] Dashboard mode stopped")
    ...
    async def run_trading_mode(self):
        ...
        try:
            self.logger.info("[START] BEAST MODE TRADING BOT STARTED")
            self.logger.info(f"[STATS] Trading Mode: {'LIVE' if self.live_mode else 'PAPER'}")
            self.logger.info(f"[MONEY] Daily AI Budget: ${settings.trading.daily_ai_budget}")
            self.logger.info(f"[FAST] Features: Market Making + Portfolio Optimization + Dynamic Exits")
            
            # [ALERT] CRITICAL FIX: Initialize database FIRST and wait for completion
            self.logger.info("[FIX] Initializing database...")
            db_manager = DatabaseManager()
            await self._ensure_database_ready(db_manager)
            self.logger.info("[OK] Database initialization complete!")
            ...
            self.logger.info("[REFRESH] Starting market ingestion...")
            ...
            self.logger.info("[START] Starting trading and monitoring tasks...")
            tasks = [
            ...
            self.logger.info("[END] Beast Mode Bot shut down gracefully")
    ...
    async def _ensure_database_ready(self, db_manager: DatabaseManager):
        ...
        try:
            ...
            self.logger.info("[TARGET] Database tables verified and ready")
        except Exception as e:
    ...
    async def _run_market_ingestion(self, db_manager: DatabaseManager, kalshi_client: KalshiClient):
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                # [OK] FIXED: Pass the shared database manager
    ...
    async def _run_trading_cycles(self, db_manager: DatabaseManager, kalshi_client: KalshiClient, xai_client: XAIClient):
        ...
        while not self.shutdown_event.is_set():
            try:
                ...
                cycle_count += 1
                self.logger.info(f"[REFRESH] Starting Beast Mode Trading Cycle #{cycle_count}")
                ...
                if results and results.total_positions > 0:
                    self.logger.info(
                        f"[OK] Cycle #{cycle_count} Complete - "
                    ...
                    self.logger.info(f"[STATS] Cycle #{cycle_count} Complete - No new positions created")
    ...
    async def _check_daily_ai_limits(self, xai_client: XAIClient) -> bool:
        ...
        if hasattr(xai_client, 'daily_tracker') and xai_client.daily_tracker.is_exhausted:
            self.logger.warning(
                "[BLOCK] Daily AI cost limit reached - trading paused",
    ...
    async def _sleep_until_next_day(self):
        ...
        if sleep_time > 0:
            ...
            self.logger.info(
                f"[SLEEP] Sleeping until next day to reset AI limits - {hours_to_sleep:.1f} hours"
            )
...
async def main():
    ...
    parser = argparse.ArgumentParser(
        description="Beast Mode Trading Bot [START] - Advanced Multi-Strategy Trading System",
...
Beast Mode Features:
  ...
  • Cost controls and budget management
    ...
    if args.live and not args.dashboard:
        ...
        print("[MONEY] This will use real money and place actual trades!")
        print("[START] LIVE TRADING MODE CONFIRMED")
...
if __name__ == "__main__":
    try:
        ...
        print("\n[WAVE] Beast Mode Bot stopped by user")
    except Exception as e:
        print(f"[FAIL] Beast Mode Bot error: {e}")
        raise 
