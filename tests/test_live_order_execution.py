...
async def test_immediate_trading_fix():
    ...
    try:
        ...
        logger.info("[STATS] Checking initial Kalshi positions...")
        ...
        if not tradeable_markets:
            ...
            logger.info("[INFO] This is expected during off-hours or when markets are closed")
            return False
        ...
        logger.info("[SAVE] Checking database for new positions...")
        import aiosqlite
        ...
        if db_positions:
            ...
            logger.info(f"[WARN] No positions found in database for {ticker}")
        
        # 7. Check Kalshi for new positions  
        logger.info("[STATS] Checking Kalshi for new positions...")
...
if __name__ == "__main__":
    ...
    if result:
        print("[WIN] IMMEDIATE TRADING FIX SUCCESSFUL! Real orders are being placed.")
    else:
