...
async def test_direct_order_placement():
    ...
    try:
        ...
        logger.info(f"[STATS] Initial position in {ticker}: {initial_position} contracts")
        ...
        async with aiosqlite.connect(db_manager.db_path) as db:
            ...
            if result:
                ...
                logger.info(f"[SAVE] Database position: live={db_live}, status={db_status}")
            else:
        ...
        if position_change != 0:
            logger.info(f"[WIN] SUCCESS! Position changed by {position_change} contracts")
...
if __name__ == "__main__":
    ...
    if result:
        print("[WIN] DIRECT ORDER TEST PASSED! Real orders are being placed on Kalshi.")
    else:
