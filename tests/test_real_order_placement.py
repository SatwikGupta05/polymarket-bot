...
async def test_order_placement_flow():
    ...
    try:
        ...
        logger.info("[STATS] Getting orderbook data...")
        ...
        # 6. Cancel the order
        logger.info(f"[DELETE] Cancelling order {order_id}...")
        ...
        if cancelled_order:
            ...
            if status == 'canceled':
                ...
                logger.warning(f"[WARN] Order status: {status} (may still be processing)")
                return True
        else:
...
async def test_database_sync():
    ...
    logger.info("[REFRESH] Testing database synchronization with Kalshi portfolio")
    ...
    try:
        ...
        logger.info(f"[STATS] Kalshi positions: {len(kalshi_markets)} markets")
        logger.info(f"[SAVE] Database positions: {len(db_positions)} positions")
        ...
        for market_id, quantity, side in db_positions:
            ...
            if kalshi_pos != expected_pos:
                logger.warning(f"[WARN] MISMATCH: {market_id} - DB: {expected_pos}, Kalshi: {kalshi_pos}")
                mismatches += 1
...
if __name__ == "__main__":
    async def run_all_tests():
        ...
        logger.info("[TEST] Running order placement and sync tests...")
        ...
        if placement_success and sync_success:
            print("[WIN] ALL TESTS PASSED! Order placement is working correctly.")
            return True
        else:
