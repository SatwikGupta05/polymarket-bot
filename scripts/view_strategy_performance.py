...
async def view_strategy_performance():
    ...
    print("[STATS] STRATEGY PERFORMANCE ANALYSIS")
    print("=" * 60)
    ...
    try:
        ...
        if not performance:
            print("[INFO]  No strategy performance data available yet.")
        ...
        for strategy_name, stats in sorted_strategies:
            print(f"\n[LIST] {strategy_name.upper().replace('_', ' ')}")
        ...
        if total_trades > 0:
            ...
            print("[INFO] INSIGHTS:")
            print(f"   [TROPHY] Best P&L: {best_strategy[0]} (${best_strategy[1]['total_pnl']:.2f})")
...
async def view_recent_positions_by_strategy():
    ...
    print("\n[LIST] RECENT POSITIONS BY STRATEGY")
    print("=" * 60)
    ...
    try:
        ...
        if not positions:
            print("[INFO]  No positions with strategy information found.")
            return
