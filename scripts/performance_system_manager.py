...
class PerformanceSystemManager:
    ...
    async def start_system(self, config: Optional[ScheduleConfig] = None) -> None:
        ...
        self.logger.info("[START] Starting Polymarket Performance Analysis System")
        ...
        try:
            ...
            print(f"\n{'='*60}")
            print("[TARGET] POLYMARKET PERFORMANCE ANALYSIS SYSTEM ACTIVE")
            print(f"{'='*60}")
            ...
            if initial_summary['alerts']:
                ...
                for alert in initial_summary['alerts']:
                    priority_icon = "[ALERT]" if alert['priority'] == 'CRITICAL' else "[WARN]"
    ...
    async def run_immediate_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            print(f"\n{'='*60}")
            print("[STATS] IMMEDIATE ANALYSIS RESULTS")
            print(f"{'='*60}")
    ...
    async def emergency_intervention(self) -> None:
        ...
        try:
            ...
            print(f"\n[STATS] EMERGENCY ANALYSIS RESULTS:")
  ...
  python performance_system_manager.py --interactive
    ...
    try:
        if args.start:
            ...
            parser.print_help()
            print(f"\n[INFO] Quick start: python {parser.prog} --start")
