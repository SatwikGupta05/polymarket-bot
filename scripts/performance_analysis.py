...
async def comprehensive_analysis():
    ...
    print("[TARGET] POLYMARKET TRADING SYSTEM PERFORMANCE ANALYSIS")
    print("=" * 60)
    ...
    try:
        ...
        print("\n[STATS] CURRENT POSITIONS ANALYSIS")
        print("-" * 40)
        ...
        print(f"[OK] Active Polymarket Positions: {len(non_zero_positions)} markets")
        ...
        # Calculate total position value from the exchange
        ...
        for db_pos in open_positions:
            ...
            if kalshi_pos:
                ...
                if kalshi_quantity != expected_quantity:
                    discrepancies += 1
                    print(f"[WARN]  {market_id}: DB={expected_quantity}, Polymarket={kalshi_quantity}")
        
        if discrepancies == 0:
            print("[OK] All database positions match Polymarket positions")
        else:
        ...
        if len(non_zero_positions) == 0:
            ...
            print("[INFO] Suggestion: Check eligibility filters and confidence thresholds")
        elif len(non_zero_positions) < 5:
            print("[STATS] Status: LOW ACTIVITY - Conservative trading approach")
            print("[INFO] Suggestion: Consider expanding eligibility criteria")
        else:
        ...
        try:
            async with aiosqlite.connect(db.db_path) as database:
                ...
                if win_rate is not None:
                    ...
                    if win_rate < 0.4:
                        print("[WARN]  Low win rate - consider adjusting strategy")
                    elif win_rate > 0.6:
                        print("[OK] Strong win rate - system performing well")
                        
        except Exception as e:
            print("[STATS] Win rate data not available yet")
