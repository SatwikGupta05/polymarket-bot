...
class BeastModeDashboard:
    ...
    def __init__(self):
        self.db_manager = DatabaseManager(str(DASHBOARD_DB_PATH))
    ...
    async def show_live_dashboard(self):
        ...
        try:
            while True:
                ...
                print("\n" + "=" * 60)
                print("[REFRESH] Updates every 30 seconds | Ctrl+C to exit")
                
                # Wait 30 seconds before refresh
                await asyncio.sleep(30)
                
        except KeyboardInterrupt:
            print("\n\n[WAVE] Dashboard stopped. Thanks for using Beast Mode!")
    ...
    async def _display_portfolio_overview(self, performance: Dict):
        ...
        try:
            ...
            print(f"[DATE] Today's Trades: {recent_trades.get('trades_today', 0)}")
            print(f"[USD] Today's P&L: ${recent_trades.get('pnl_today', 0):+.2f}")
            print(f"[STATS] Win Rate (7d): {recent_trades.get('win_rate_7d', 0):.1%}")
    ...
    async def _display_risk_metrics(self, performance: Dict):
        ...
        try:
            ...
            print(f"[RANDOM] Portfolio Concentration: {concentration_risk:.1f}%")
            print(f"[SHIELD]  Exit Strategy Coverage: {exit_coverage:.0f}%")
            print(f"[TIMER] Avg Time to Expiry: {self._calculate_avg_time_to_expiry(positions):.1f} days")
            print(f"[REFRESH] Diversification Score: {self._calculate_diversification_score(positions):.2f}")
    ...
    async def _display_cost_analysis(self, performance: Dict):
        ...
        try:
            ...
            print(f"[STATS] Budget Status: {status}")
            print(f"[REFRESH] Analysis Count Today: {cost_analysis.get('analyses_today', 0)}")
            print(f"[INFO] Avg Cost per Analysis: ${cost_analysis.get('avg_cost_per_analysis', 0):.3f}")
    ...
    async def _display_system_health(self, performance: Dict):
        ...
        try:
            ...
            print(f"[STATS] Available Markets: {available_markets}")
            print(f"🔌 System Status: {'[GREEN] OPERATIONAL' if available_markets > 0 else '[RED] LIMITED'}")
    ...
    async def show_summary(self):
        ...
        print(f"[STATS] Active Positions: {len(positions)}")
