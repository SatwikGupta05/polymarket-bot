#!/usr/bin/env python3
"""
Cost Monitor - Real-time AI spending tracker for the Polymarket trading system
    ...
    if summary['totals'] and summary['totals'][0]:
        ...
        if days == 1:
            ...
            if today_data:
                ...
                status = "[GREEN]" if budget_used < 0.5 else "[YELLOW]" if budget_used < 0.8 else "[RED]"
                print(f"\n{status} Today's Budget: ${today_cost:.3f} / ${summary['budget_limit']} ({budget_used:.1%})")
    else:
        print("[STATS] No cost data available for this period")
...
async def live_monitor(db_manager: DatabaseManager):
    ...
    print("[RED] LIVE MONITORING MODE (Ctrl+C to exit)")
    ...
    try:
        while True:
            ...
            # Show current time
            print(f"[TIME] Last updated: {datetime.now().strftime('%H:%M:%S')}")
            print("   Press Ctrl+C to exit")
            
            await asyncio.sleep(30)
            
    except KeyboardInterrupt:
        print("\n[WAVE] Monitoring stopped")

async def main():
    parser = argparse.ArgumentParser(description="Monitor AI costs for the Polymarket trading system")
