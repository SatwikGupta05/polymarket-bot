...
async def test_automated_analyzer():
    ...
    try:
        ...
        print(f"[STATS] SYSTEM HEALTH SCORE: {summary['overall_health_score']:.1f}/100")
        print(f"[RED] Critical Issues: {summary['total_critical_issues']}")
        print(f"[WARN]  Warnings: {summary['total_warnings']}")
        ...
        if 'grok_analysis' in report and report['grok_analysis']['status'] == 'success':
            ...
            print(f"\n[WARN]  Grok4 analysis unavailable, using fallback analysis")
        ...
        print(f"\n[SAVE] Full report saved to database and JSON file")
        print("=" * 60)
