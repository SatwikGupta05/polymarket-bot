...
def print_portfolio_summary(health: Dict):
    ...
    # Financial overview
    print("[USD] FINANCIAL OVERVIEW")
    print("-" * 30)
    print(f"[MONEY] Total Portfolio Value:  ${health['total_portfolio_value']:.2f}")
    print(f"[USD] Available Cash:         ${health['available_cash']:.2f}")
    print(f"[STATS] Position Value:         ${health['position_value']:.2f}")
    print()
    
    # Utilization metrics
    print("[UP] UTILIZATION METRICS")
    print("-" * 30)
    print(f"[STATS] Portfolio Utilization:  {health['utilization_pct']:.1f}%")
    print(f"[MONEY] Cash Reserve:           {health['cash_pct']:.1f}%")
    ...
    if health['cash_pct'] < 10:
        print("[RED] LOW CASH: Consider closing some positions to free up trading capital")
    elif health['cash_pct'] < 20:
        print("[YELLOW] MODERATE CASH: Watch cash levels for new opportunities")
    else:
        print("[GREEN] HEALTHY CASH: Good liquidity for new trades")
    
    if health['utilization_pct'] > 90:
        print("[RED] HIGH UTILIZATION: Portfolio heavily invested, limited flexibility")
    elif health['utilization_pct'] > 70:
        print("[YELLOW] MODERATE UTILIZATION: Well-invested but still some flexibility")
    else:
        print("[GREEN] CONSERVATIVE UTILIZATION: Room for additional positions")
...
def print_position_details(health: Dict):
    ...
    if not health['position_details']:
        print("[STATS] No active positions")
        return
    
    print("[STATS] POSITION DETAILS")
    print("-" * 80)
...
async def main():
    ...
    try:
        ...
        print("[INFO] TRADING RECOMMENDATIONS")
        print("-" * 40)
