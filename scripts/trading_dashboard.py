...
DASHBOARD_DB_PATH = PROJECT_ROOT / "trading_system.db"
...
def load_performance_data():
    ...
    try:
        ...
        db_manager = DatabaseManager(str(DASHBOARD_DB_PATH))
        ...
        async def get_data():
            ...
            # Get LIVE positions from exchange API first.
            positions_response = await kalshi_client.get_positions()
            kalshi_positions = positions_response.get('market_positions', [])
            
            # Convert API positions to simple dictionaries for caching
            positions = []
            ...
            for pos in kalshi_positions:
                if pos.get('position', 0) != 0:  # Only active positions
                    ...
                    seen_position_keys.add((position_dict['market_id'], position_dict['side']))
                    positions.append(position_dict)

            # Always include DB-tracked positions so paper trades remain visible
            # even if they were marked live during simulated execution.
            db_positions = await db_manager.get_open_positions()
            for p in db_positions:
                position_key = (str(p.market_id), str(p.side))
                ...
                positions.append({
                    ...
                    'strategy': p.strategy or 'paper',
                    'status': p.status or 'open',
                    'source': 'paper_db' if not p.live else 'db_synced',
...
def load_trade_log_data():
    """Load closed trade logs from the database for dashboard display."""
    try:
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        ...
        trade_logs = loop.run_until_complete(get_data())
        loop.close()
        return trade_logs

    except Exception as e:
        st.error(f"Error loading trade log data: {e}")
        return []
...
def load_system_health():
    ...
    try:
        ...
        async def get_health():
            ...
            # Calculate current value of all positions from API positions
                ...
                try:
                    ...
                    if ticker and position_count != 0:
                        ...
                        if market_data and 'market' in market_data:
                            ...
                            # Compatibility payload: positive position = YES, negative = NO
                            if position_count > 0:  # YES position
            ...
            # Paper/read-only fallback: if API has no positions, use local DB open positions.
            if positions_count == 0:
                db_positions = await db_manager.get_open_positions()
                positions_count = len(db_positions)
                total_position_value = sum(float(p.entry_price) * int(p.quantity) for p in db_positions)
...
def main():
    ...
    with col2:
        if st.button("[REFRESH] Refresh Data", help="Clear cache and reload all data"):
            st.cache_data.clear()
            st.rerun()
    ...
    try:
        ...
        trade_logs = load_trade_log_data()
    ...
    st.sidebar.markdown("---")
    st.sidebar.markdown("**[STATS] Data Status:**")
    st.sidebar.metric("Active Positions", len(positions) if positions else 0)
    st.sidebar.metric("Closed Trades", len(trade_logs) if trade_logs else 0)
...
def show_overview(performance_data, positions, system_health_data):
    ...
    with col3b:
        ...
        if system_health_data['total_portfolio_value'] > 0:
            ...
            utilization_pct = 0
        st.metric(
            label="[STATS] Portfolio Utilization",
    ...
    if performance_data:
        ...
        st.info("[STATS] **No strategy data yet** - Run the trading system to start collecting performance data")
    
    # Recent activity summary
    st.subheader("[LIST] Recent Activity")
...
def show_strategy_performance(performance_data):
    ...
    if selected_strategy == "All Strategies":
        ...
        st.subheader("[STATS] Strategy Comparison")
        ...
        st.subheader(f"[LIST] {selected_strategy.replace('_', ' ').title()} Performance")
...
def show_llm_analysis(llm_queries, llm_stats):
    ...
    st.header("[BOT] LLM Analysis & Review")
    ...
    if not llm_queries and not llm_stats:
        ...
        st.info("[INFO] **Tip:** The system will automatically log all future Grok queries for analysis.")
        return
    
    # LLM usage stats
    if llm_stats:
        st.subheader("[STATS] LLM Usage Statistics (Last 7 Days)")
    ...
    if llm_queries:
        ...
        for i, query in enumerate(filtered_queries[:20]):  # Show latest 20
            with st.expander(
                f"[BOT] {query.strategy} | {query.query_type} | {query.timestamp.strftime('%H:%M:%S')}",
...
def show_positions_trades(positions, trade_logs):
    """Show detailed positions and trades analysis."""
    
    st.header("💼 Positions & Trades")
    
    if not positions and not trade_logs:
        st.warning("No active positions or closed trades found.")
        return

    if not positions:
        st.info("No active positions currently.")
        trade_rows = []
        for trade in sorted(trade_logs, key=lambda item: item['exit_timestamp'], reverse=True):
            try:
                entry_time = datetime.fromisoformat(trade['entry_timestamp']).strftime('%m/%d %H:%M')
            except Exception:
                entry_time = 'Unknown'

            try:
                exit_time = datetime.fromisoformat(trade['exit_timestamp']).strftime('%m/%d %H:%M')
            except Exception:
                exit_time = 'Unknown'
            ...
            trade_rows.append({
                ...
                'Entry Price': f"${trade['entry_price']:.3f}",
                'Exit Price': f"${trade['exit_price']:.3f}",
                'PnL': f"${trade['pnl']:.2f}",
                'Entry Time': entry_time,
                'Exit Time': exit_time,
                'Rationale': trade['rationale'][:120] if trade['rationale'] else ""
            })

        st.subheader(f"Closed Trades ({len(trade_logs)})")
        st.dataframe(pd.DataFrame(trade_rows), use_container_width=True, hide_index=True)
        return
    
    # Positions overview
    st.subheader(f"[STATS] Active Positions ({len(positions)})")
...
def show_risk_management(performance_data, positions, system_balance):
    ...
    try:
        ...
        if portfolio_utilization > 90:
            alerts.append("[WARN] **High Portfolio Utilization**: Over 90% of capital deployed")
        
        if max_risk_pct > 20:
            alerts.append("[WARN] **Large Position Risk**: Single position exceeds 20% of portfolio")
        
        if len(positions) > 50:
            alerts.append("[WARN] **High Position Count**: Over 50 active positions may be difficult to manage")
        ...
        if no_stop_loss:
            alerts.append(f"[WARN] **No Stop Losses**: {len(no_stop_loss)} positions lack stop loss protection")
        ...
        if len(set(strategy_names)) > 1:
            st.subheader("[STATS] Risk by Strategy")
...
def show_system_health(available_cash, positions_count, llm_stats):
    ...
    st.header("[FIX] System Health")
    
    # System status
    st.subheader("[GREEN] System Status")
    
    col1, col2, col3 = st.columns(3)
    
    with col1:
        st.success("[OK] **Polymarket Connection**: Active")
    ...
    with col2:
        if llm_stats:
            ...
            st.warning("[WARN] **LLM Logging**: No data")
    ...
    st.subheader("[DATE] System Activity")
    ...
    config_info = {
        "Database Path": str(DASHBOARD_DB_PATH),
    ...
    st.subheader("[INFO] Recommendations")
    ...
    if not llm_stats:
        recommendations.append("[BOT] LLM query logging will begin with next trading cycle")
    
    total_queries = sum(stats['query_count'] for stats in llm_stats.values()) if llm_stats else 0
    if total_queries > 1000:
        recommendations.append("[STATS] High LLM usage - consider optimizing query frequency")
